...
/*
  def freshenClasses(enclosingFn: FunDef, locals: Seq[(ClassDef, Seq[FunDef], Seq[TypeDef])]): (FunDef, Seq[(ClassDef, Seq[FunDef], Seq[TypeDef])]) =
    locals.foldLeft((enclosingFn, Seq.empty[(ClassDef, Seq[FunDef], Seq[TypeDef])])) {
      case ((enclosingFnAcc, localsAcc), (cd, meths, tds)) =>
        val (newEnclosingFn, newCd, newMeths, newTds) = freshenClass(enclosingFnAcc, cd, meths, tds)
        (newEnclosingFn, localsAcc :+ (newCd, newMeths, newTds))
    }
*/
