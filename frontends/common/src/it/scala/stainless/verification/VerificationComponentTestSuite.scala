...
trait VerificationComponentTestSuite extends ComponentTestSuite { self =>
  ...
  def testPosAll(dir: String, recursive: Boolean = false, discard: String => Boolean = _ => false): Unit =
    testAll(dir, recursive, discard) { (analysis, reporter, _) =>
  ...
  // TODO: also have a testUnkownAll or something like that
  // TODO: Comment
  ...
  def testNegAll(dir: String, discard: String => Boolean = _ => false): Unit = {
    val allCheckFiles = resourceFiles(dir,
      f => f.contains(".check") && (!f.contains(".scalac.check") || isScala2) && (!f.contains(".dotty.check") || isScala3),
      recursive = false)

    testAll(dir, recursive = false, discard) { (analysis, reporter, unit) =>
