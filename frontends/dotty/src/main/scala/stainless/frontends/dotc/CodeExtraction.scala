...
//       ignore
  ...
  private def extractAccessor(classType: xt.ClassType | xt.LocalClassType, vd: tpd.ValDef, fieldPos: Int)(using dctx: DefContext): Seq[xt.FunDef] = {
    ...
    vd match {
      ...
      case ExNonCtorFieldDef(_, _, _) if vd.rhs != tpd.EmptyTree =>
...
*/
    ...
    case Inlined(call, members, body) =>
      def rec(expr: xt.Expr): xt.Expr = expr match {
        ...
        case _ =>
          xt.Annotated(expr, Seq(xt.DropVCs)).copiedFrom(expr)
      }
    ...
    */
    ...
    // TODO: Also say that reaching ExCall does not mean that it's not a "this" call, but that the extracted `rec` contains a This(<cls-sym>)
